...
class RecipeApp(App):
    def build(self):
        self.box_layout = BoxLayout(orientation='vertical', padding=[20,60], spacing=10)
        self.recipe_name_input = TextInput(hint_text='Recipe Name', size_hint=(.5, .1), pos_hint={'center_x': .5})
        self.recipe_type_spinner = Spinner(text='Select Recipe Type', values=('Breakfast', 'Lunch', 'Dinner', 'Dessert'), size_hint=(.5, 0.1), pos_hint={'center_x': .5})
        self.recipe_ingredients_input = TextInput(hint_text='Recipe Ingredients', size_hint=(.5, .1), pos_hint={'center_x': .5})
        self.recipe_instructions_input = TextInput(hint_text='Recipe Instructions', size_hint=(.5, .1), pos_hint={'center_x': .5})
        self.register_button = Button(text='Register Recipe', size_hint=(.5, 0.1), pos_hint={'center_x': .5})
